...
import com.example.poetryline.detail.VoiceActivity;
import com.example.poetryline.detail.manager.MediaManager;
import com.example.poetryline.detail.utils.CommonsUtils;
...
public class ExampleAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            //时间
            viewHolder.date = (TextView)convertView.findViewById(R.id.iea_date);
        ...
        String path1 = "/data/data/com.example.poetryline/files/poetryLinePic/"+record.getUserPhone()+".jpg";
        @SuppressLint("SdCardPath") File file = new File(path1);
        Bitmap bitmap = BitmapFactory.decodeFile(path1);
        if(file.exists()){
            viewHolder.ieaHeadImg.setImageBitmap(bitmap);
        }else {
            //放下载头像函数
            viewHolder.ieaHeadImg.setImageDrawable(mContext.getResources().getDrawable(R.drawable.meinv));
        }
